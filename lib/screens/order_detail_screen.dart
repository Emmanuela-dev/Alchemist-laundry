...
class _OrderDetailScreenState extends State<OrderDetailScreen> {
  ...
  void initState() {
    super.initState();
    order = MockRepo.instance.getOrder(widget.orderId);
    comments = MockRepo.instance.getComments(widget.orderId);
  }

  void _addComment() async {
    final user = MockRepo.instance.currentUser!;
    await MockRepo.instance.addComment(widget.orderId, user.id, _comment.text, _rating);
    setState(() {
      comments = MockRepo.instance.getComments(widget.orderId);
      _comment.clear();
      _rating = 5;
  ...
  Widget build(BuildContext context) {
    ...
    final service = MockRepo.instance.listServices().firstWhere((s) => s.id == order!.serviceId);
    return Scaffold(
